#!/usr/bin/env python3
"""Add interpolated correction values from a reference file to a calibration file.

Usage: python adjust_cal.py <calibration_file> <reference_file> <start_freq> [--fade-in]

Arguments:
  calibration_file  File to adjust (overwritten in place)
  reference_file    Reference correction data (e.g. 0over90.txt)
  start_freq        Frequency (Hz) at which full correction begins

Options:
  --fade-in         Linearly fade correction from 20 Hz (0%) to start_freq (100%).
                    Without this flag, frequencies below start_freq are left intact.
"""
import sys
import argparse
import numpy as np

parser = argparse.ArgumentParser(description="Add interpolated correction to a calibration file.")
